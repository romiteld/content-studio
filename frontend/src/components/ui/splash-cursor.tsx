...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getSupportedFormat(gl: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number): any {
      if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
        ...
        switch (internalFormat) {
          case (gl2.R16F || 0x822D):
            return getSupportedFormat(gl, (gl2.RG16F || 0x822F), (gl2.RG || 0x8227), type);
          case (gl2.RG16F || 0x822F):
            return getSupportedFormat(gl, (gl2.RGBA16F || 0x881A), gl.RGBA, type);
          default:
            return null;
        }
      }
    ...
    function drawDisplay(target: any): void {
    ...
    function splatPointer(pointer: any): void {
      let dx = pointer.deltaX * config.SPLAT_FORCE;
      let dy = pointer.deltaY * config.SPLAT_FORCE;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color);
    }

    function clickSplat(pointer: any): void {
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: any): void {
      splatProgram.bind();
    ...
    function updatePointerDownData(pointer: any, id: number, posX: number, posY: number): void {
      pointer.id = id;
      pointer.down = true;
      pointer.moved = false;
    ...
    function updatePointerMoveData(pointer: any, posX: number, posY: number, color?: any): void {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas!.width;
      pointer.texcoordY = 1.0 - posY / canvas!.height;
    ...
    function updatePointerUpData(pointer: any): void {
      pointer.down = false;
    }
    ...
    function generateColor() {
      // Brand colors: Gold (#D4AF37) and Cyan (#2EA3F2)
      const brandColors = [
        { r: 0.831, g: 0.686, b: 0.216 }, // Gold #D4AF37
        { r: 0.180, g: 0.639, b: 0.949 }, // Cyan #2EA3F2
        { r: 0.745, g: 0.608, b: 0.267 }, // Light Gold #BE9E44
        { r: 0.310, g: 0.761, b: 0.969 }  // Light Cyan #4FC3F7
      ];
      
      const selectedColor = brandColors[Math.floor(Math.random() * brandColors.length)];
      
      // Make colors more subtle for background effect
      return {
        r: selectedColor.r * 0.25,
        g: selectedColor.g * 0.25,
        b: selectedColor.b * 0.25
      };
    }
